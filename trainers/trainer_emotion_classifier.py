"""
Trainer for Emotion Classifier.
"""
...
class EmotionClassifierTrainer(BaseTrain):
    def __init__(self, sess, model, config, logger, data_loader):
        ...
        """
        super().__init__(sess, model, config, logger, data_loader)
...
Epoch-{} Train loss:{:.4f} -- acc:{:.4f}
    ...
    def test(self, epoch=1, mode='dev'):
        # initialize dataset
        self.data_loader.initialize(self.sess, mode)

        # initialize tqdm
        tt = tqdm(range(self.data_loader.num_iterations_dev), total=self.data_loader.num_iterations_dev,
